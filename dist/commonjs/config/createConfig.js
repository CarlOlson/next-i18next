  ...
  if (!process.browser && typeof window === 'undefined') {
    ...
    if (!hasCustomBackend) {
      ...
      if (typeof defaultNS === 'string' && typeof lng !== 'undefined') {
        ...
        var defaultNSPath = _path["default"].join(localePath, defaultFile);

        var defaultNSExists = _fs["default"].existsSync(defaultNSPath);
      ...
      combinedConfig.backend = {
        addPath: _path["default"].resolve(process.cwd(), "".concat(serverLocalePath, "/").concat(localeStructure, ".missing.").concat(localeExtension)),
        loadPath: _path["default"].resolve(process.cwd(), "".concat(serverLocalePath, "/").concat(localeStructure, ".").concat(localeExtension))
      }; //
      ...
      if (!combinedConfig.ns) {
        var getAllNamespaces = function getAllNamespaces(p) {
          return _fs["default"].readdirSync(p).map(function (file) {
            return file.replace(".".concat(localeExtension), '');
          });
        };

        combinedConfig.ns = getAllNamespaces(_path["default"].resolve(process.cwd(), "".concat(serverLocalePath, "/").concat(lng)));
      }
    }
  } else {
